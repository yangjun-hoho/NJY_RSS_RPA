...
def main():
    ...
    .main-title {
        ...
        text-align: left;
        background: linear-gradient(15deg, #167331, #E2F7E2); /* 더 연한 파란색 그라데이션 */
        color: white;
    ...
    # 사이드바에 앱 선택 옵션 추가 - 방명록 추가
    st.sidebar.markdown('<p class="sidebar-text">📱 애플리케이션 선택</p>', unsafe_allow_html=True)
    app_choice = st.sidebar.selectbox(
        "",  # 레이블을 비워둠 (위에서 마크다운으로 스타일 적용)
        ["(생성형AI) 인사말씀 생성기", "(생성형AI) 문서자료 대본 변환기", "(생성형AI) 보도자료 생성기", "(NYJ_RPA) 폐기물스티커 판매정산", "(NYJ_RPA) FAX 보내기", "방명록"]
    )
